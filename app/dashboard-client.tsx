...
type BootstrapContact = Omit<StoredContact, "lastContact"> & {
  email?: string | null;
  phone?: string | null;
  company?: string | null;
...
const writeJsonCache = (key: string | null, value: unknown) => {
  if (!key || typeof window === "undefined") {
    return;
  }
  try {
...
const normalizeCircleSettings = (circles: BootstrapCircle[]): CircleSetting[] => {
  ...
  return padded.length > 0 ? padded : getDefaultCircleSettings();
};
...
export default function DashboardClient() {
  ...
  const {
    value: cachedCircleSettings,
    setValue: setCachedCircleSettings,
  ...
  const liveContactsStorageKey = session?.user?.email
    ? `${CONTACTS_CACHE_PREFIX}${session.user.email}`
    : null;
  const [dbContacts, setDbContacts] = useState<BootstrapContact[]>([]);
  const [liveCircleSettings, setLiveCircleSettings] = useState<CircleSetting[]>(
    getDefaultCircleSettings()
  );
  const [hasLoadedLiveCache, setHasLoadedLiveCache] = useState(false);
  const contactsDirtyRef = useRef(false);
  const circlesDirtyRef = useRef(false);
  const circleSettings = isLiveMode ? liveCircleSettings : cachedCircleSettings;
  const [draftCircleSettings, setDraftCircleSettings] = useState<CircleSetting[]>(
    circleSettings
  );
  const setLiveContacts = useCallback(
    (
      nextValue:
        | BootstrapContact[]
        | ((current: BootstrapContact[]) => BootstrapContact[])
    ) => {
      contactsDirtyRef.current = true;
      setDbContacts((current) => {
        const resolved =
          typeof nextValue === "function"
            ? (nextValue as (currentValue: BootstrapContact[]) => BootstrapContact[])(
                current
              )
            : nextValue;
        writeJsonCache(liveContactsStorageKey, resolved);
        return resolved;
      });
    },
    [liveContactsStorageKey]
  );
  const setCircleSettings = useCallback(
    ...
    [isLiveMode, setCachedCircleSettings]
  );
  const isLiveContactsReady = isLiveMode ? hasLoadedLiveCache : true;
  ...
  useEffect(() => {
    ...
    circlesDirtyRef.current = false;
    setHasLoadedLiveCache(false);
  }, [liveContactsStorageKey]);

  useEffect(() => {
    if (!isLiveMode || !liveContactsStorageKey) {
      setHasLoadedLiveCache(true);
      return;
    }
    const cachedContacts = readJsonCache<BootstrapContact[]>(
      liveContactsStorageKey,
      []
    );
    if (!contactsDirtyRef.current) {
      setDbContacts(cachedContacts);
    }
    setHasLoadedLiveCache(true);
  }, [isLiveMode, liveContactsStorageKey]);

  useEffect(() => {
    if (!isLiveMode || !areCircleSettingsLoaded) {
      return;
    }
    if (!circlesDirtyRef.current) {
      setLiveCircleSettings(cachedCircleSettings);
    }
  }, [areCircleSettingsLoaded, cachedCircleSettings, isLiveMode]);

  useEffect(() => {
    if (!isLiveMode || !hasLoadedLiveCache) {
      return;
    }
    ...
    fetch("/api/bootstrap", { signal: controller.signal })
      .then(async (response) => {
        if (!response.ok) {
          throw new Error("Failed to fetch bootstrap payload");
        }
      ...
      .then((payload) => {
        ...
        if (!contactsDirtyRef.current) {
          setDbContacts(payload.contacts);
          writeJsonCache(liveContactsStorageKey, payload.contacts);
        }
        if (!circlesDirtyRef.current) {
          const nextCircles = normalizeCircleSettings(payload.circles);
          setLiveCircleSettings(nextCircles);
          setCachedCircleSettings(nextCircles);
        }
      })
      .catch((error) => {
        if (!controller.signal.aborted) {
  ...
  const addContact = useCallback(
    async (input: CreateContactInput) => {
      ...
      const optimisticContact: BootstrapContact = {
        ...
        daysAgo: input.lastContact ? getDaysAgoFromDate(input.lastContact) : null,
        nextMeetDate: input.nextMeetDate ?? null,
        nextMeetCadence: input.nextMeetCadence ?? null,
        isQuick: input.isQuickContact,
        isQuickContact: input.isQuickContact,
            ...
            ? {
                ...
                daysAgo:
                  input.lastContact !== undefined
                    ? input.lastContact
                      ? getDaysAgoFromDate(input.lastContact)
                      : null
                    : contact.daysAgo,
                notes:
